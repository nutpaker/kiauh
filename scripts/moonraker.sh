...
function install_moonraker_dependencies() {
  ...
  # Phak By Pass Install Package => libcurl4-openssl-dev
...
function moonraker_setup() {
  ...
  # Phak Disable  Clone Moonraker
  clone_moonraker "${MOONRAKER_REPO}"
  # Phak By Pass
  local removed
  status_msg "1. Open -> ${MOONRAKER_DIR}/script/moonraker-requiremts.txt"
  status_msg "2. comment line pycurl with #"
  while true; do
     read -p "${cyan} ##### Editabled (Y/n):${white} " removed
    case "${removed}" in
      Y|y|Yes|yes|"")
        select_msg "Yes"
        break;;
